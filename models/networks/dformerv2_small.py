...
class dformerv2(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            hit_count = len(model_keys & set(state_dict.keys()))
            print(f"[DFormerv2] loading pretrained from {pretrained}, matched_keys={hit_count}/{len(model_keys)}")
...
class DFormerv2SmallSeg(nn.Module):

    def __init__(self, in_chns=3, class_num=2, pretrain_path=None):
        super().__init__()
        self.params = {"in_chns": in_chns, "class_num": class_num, "pretrain_path": pretrain_path}
        self.encoder = DFormerv2_S()
        if pretrain_path:
            self.encoder.init_weights(pretrain_path)
        ...
        self.fuse = nn.Sequential(
            ...
            nn.Conv2d(128, class_num, kernel_size=1),
        )
    ...
    def forward(self, x):
        ...
        for idx, feat in enumerate(feats):
            ...
            fused_feats.append(self.proj[idx](feat))
        out = self.fuse(torch.cat(fused_feats, dim=1))
