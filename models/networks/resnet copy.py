...
class DepthGuiderV4(nn.Module):
    ...
    def compute_delta(self, rgb_feat, depth):
        ...
        delta = self.delta_proj(fused) * gate
        if self._vis_on:
            self._vis_cache = {
                'gate': gate.detach().cpu(),
                'scale_weight': scale_weight.detach().cpu(),
                'depth_ctx': depth_ctx.detach().cpu(),
                'delta': delta.detach().cpu(),
                'feat_norm': rgb_feat.detach().cpu().pow(2).sum(1, keepdim=True).sqrt(),
            }
        return delta
...
class DepthGuidedResNetEncoderV4(nn.Module):
    _vis_before, _vis_after, _vis_caches = [], [], []
    _vis_idx = 0
    _vis_on = True
    ...
    def forward(self, x, depth):
        for g in self.depth_guiders:
            g._vis_on = self._vis_on
        e1 = self.encoder1_relu(self.encoder1_bn(self.encoder1_conv(x)))
        if self._vis_on:
            self._vis_before.append(e1.detach().cpu())
        e1 = self.depth_guiders[0](e1, depth)
        if self._vis_on:
            self._vis_after.append(e1.detach().cpu())
            self._vis_caches.append(self.depth_guiders[0]._vis_cache)
        e2 = self.encoder2(self.maxpool(e1))
        if self._vis_on:
            self._vis_before.append(e2.detach().cpu())
        e2 = self.depth_guiders[1](e2, depth)
        if self._vis_on:
            self._vis_after.append(e2.detach().cpu())
            self._vis_caches.append(self.depth_guiders[1]._vis_cache)
        e3 = self.encoder3(e2)
        if self._vis_on:
            self._vis_before.append(e3.detach().cpu())
        e3 = self.depth_guiders[2](e3, depth)
        if self._vis_on:
            self._vis_after.append(e3.detach().cpu())
            self._vis_caches.append(self.depth_guiders[2]._vis_cache)
        e4 = self.encoder4(e3)
        if self._vis_on:
            self._vis_before.append(e4.detach().cpu())
        e4 = self.depth_guiders[3](e4, depth)
        if self._vis_on:
            self._vis_after.append(e4.detach().cpu())
            self._vis_caches.append(self.depth_guiders[3]._vis_cache)
        e5 = self.encoder5(e4)
        if self._vis_on:
            self._vis_before.append(e5.detach().cpu())
        e5 = self.depth_guiders[4](e5, depth)
        if self._vis_on:
            self._vis_after.append(e5.detach().cpu())
            self._vis_caches.append(self.depth_guiders[4]._vis_cache)
            self._vis_save_fig()
        return e1, e2, e3, e4, e5

    def _vis_save_fig(self):
        import os, random
        if random.random() > 0.01:
            self._vis_before.clear()
            self._vis_after.clear()
            self._vis_caches.clear()
            return
        n = len(self._vis_before)
        if n == 0:
            return
        ...
        DepthGuidedResNetEncoderV4._vis_idx += 1
        idx = self._vis_idx

        # --- 1. summary: channel-mean, all layers, 10 columns ---
        ncols = 10
        fig, axes = plt.subplots(n, ncols, figsize=(3.2 * ncols, 3 * n))
        if n == 1:
            axes = [axes]
        for i, (b, a) in enumerate(zip(self._vis_before, self._vis_after)):
            c = self._vis_caches[i]
            bm, am = b[0].mean(0).numpy(), a[0].mean(0).numpy()
            dm = am - bm
            gate = c['gate'][0].mean(0).numpy()
            sw = c['scale_weight'][0].numpy()
            dc = c['depth_ctx'][0].mean(0).numpy()
            delta_mean = c['delta'][0].mean(0).numpy()
            delta_norm = c['delta'][0].pow(2).sum(0).sqrt().numpy()
            feat_norm = c['feat_norm'][0, 0].numpy()
            ratio = delta_norm / (feat_norm + 1e-6)

            lo, hi = min(bm.min(), am.min()), max(bm.max(), am.max())
            titles = ['Before', 'After', 'Diff', 'Gate', 'SW_local', 'SW_pooled', 'SW_global', 'depth_ctx', 'Delta', 'Delta/Feat']
            imgs = [bm, am, dm, gate, sw[0], sw[1], sw[2], dc, delta_mean, ratio]
            cmaps = ['viridis', 'viridis', 'RdBu_r', 'magma', 'viridis', 'viridis', 'viridis', 'viridis', 'RdBu_r', 'RdBu_r']
            for j, (title, img, cm) in enumerate(zip(titles, imgs, cmaps)):
                if cm == 'RdBu_r':
                    v = abs(img).max()
                    axes[i][j].imshow(img, cmap=cm, vmin=-v, vmax=v)
                elif title in ('Before', 'After'):
                    axes[i][j].imshow(img, cmap=cm, vmin=lo, vmax=hi)
                else:
                    axes[i][j].imshow(img, cmap=cm)
                axes[i][j].set_title(f'L{i+1} {title}')
                axes[i][j].axis('off')
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'{idx:05d}_summary.png'), dpi=120, bbox_inches='tight')
        plt.close()

        # --- 2. per-layer: all channels, per-ch cols + shared cols ---
        layer_names = ['e1', 'e2', 'e3', 'e4', 'e5']
        per_ch_titles = ['Before', 'After', 'Diff']
        shared_titles = ['Gate', 'SW_local', 'SW_pooled', 'SW_global', 'depth_ctx', 'Delta', 'Delta/Feat']
        for i, (b, a) in enumerate(zip(self._vis_before, self._vis_after)):
            c = self._vis_caches[i]
            nc = b.shape[1]
            gate_hw = c['gate'][0].mean(0).numpy()
            sw_hw = c['scale_weight'][0].numpy()
            dc_hw = c['depth_ctx'][0].mean(0).numpy()
            delta_all = c['delta'][0].numpy()
            delta_norm = (delta_all ** 2).sum(0) ** 0.5
            feat_hw = c['feat_norm'][0, 0].numpy()
            ratio_hw = delta_norm / (feat_hw + 1e-6)

            bm_all, am_all = b[0].numpy(), a[0].numpy()
            dm_all = am_all - bm_all
            lo = min(bm_all.min(), am_all.min())
            hi = max(bm_all.max(), am_all.max())
            dmax = max(abs(dm_all.min()), abs(dm_all.max()))

            # left: nc rows x 3 cols (per-channel), right: 1 row x 7 cols (shared, spanning all rows)
            fig = plt.figure(figsize=(1.5 * 3 + 1.5 * 7, 0.8 * nc))
            gs = fig.add_gridspec(nc, 10, wspace=0.01, hspace=0.01)

            # per-channel columns (Before, After, Diff)
            for ch in range(nc):
                for j, (img, cm) in enumerate(zip([bm_all[ch], am_all[ch], dm_all[ch]],
                                                  ['viridis', 'viridis', 'RdBu_r'])):
                    ax = fig.add_subplot(gs[ch, j])
                    if cm == 'RdBu_r':
                        ax.imshow(img, cmap=cm, vmin=-dmax, vmax=dmax)
                    else:
                        ax.imshow(img, cmap=cm, vmin=lo, vmax=hi)
                    ax.axis('off')
                    if ch == 0:
                        ax.set_title(per_ch_titles[j])

            # shared columns (Gate, SW_local, SW_pooled, SW_global, depth_ctx, Delta, Delta/Feat)
            shared_imgs = [gate_hw, sw_hw[0], sw_hw[1], sw_hw[2], dc_hw, delta_all.mean(0), ratio_hw]
            shared_cmaps = ['magma', 'viridis', 'viridis', 'viridis', 'viridis', 'RdBu_r', 'RdBu_r']
            for j, (img, cm) in enumerate(zip(shared_imgs, shared_cmaps)):
                ax = fig.add_subplot(gs[:, 3 + j])  # span all rows
                if cm == 'RdBu_r':
                    v = abs(img).max()
                    ax.imshow(img, cmap=cm, vmin=-v, vmax=v)
                else:
                    ax.imshow(img, cmap=cm)
                ax.axis('off')
                ax.set_title(shared_titles[j])
            fig.suptitle(f'L{i+1} ({layer_names[i]})  {nc} channels', fontsize=14, y=1.0)
            plt.subplots_adjust(wspace=0.01, hspace=0.01, left=0, right=1, top=0.95, bottom=0)
            plt.savefig(os.path.join(save_dir, f'{idx:05d}_{layer_names[i]}.png'), dpi=120, bbox_inches='tight')
            plt.close()
...
class ResNetUNet_GeoRiskSPC(nn.Module):
    ...
    def forward(self, x, M_r=None):
        ...
        if M_r is not None:
            ...
            feature_pert[4] = self.perturbation(feature_pert[4], M_r)
