...
\item \textbf{Comprehensive evaluation}: Experiments on EndoVis 2017 demonstrate consistent improvements across multiple labeled data ratios (10\%, 20\%, 40\%), with GeoRisk-SPC achieving 92.15\% Dice with 20\% labeled data, outperforming existing methods by significant margins.
\end{itemize}
...
\subsection{Dataset}
We evaluate on the EndoVis 2017 \cite{allan20192017} surgical instrument segmentation dataset (Task 1), which contains 1800 images from 10 video sequences (1350 training, 450 testing). We use 4-fold cross-validation with sequence-based splitting, following the standard protocol. We evaluate under different labeled data ratios: 5\%, 10\%, 20\%, and 40\%.

\subsection{Implementation Details}
\begin{itemize}
\item Backbone: UNet \cite{ronneberger2015u} with 16 filters
\item Optimizer: Adam (lr=1e-4, weight decay=1e-4)
\item Batch size: 24 (12 labeled + 12 unlabeled)
\item Training iterations: 30000
\item Early stopping: patience=9000 iterations (30\% of max)
\item Depth maps: pre-computed monocular depth estimation (1 channel)
\item Risk map parameters: $\tau_r=0.5$, $\tau_c=0.9$, $\lambda_c=0.5$
\item Perturbation: dropout rate=0.3, noise std=0.1
\item Loss weights: $\lambda_{pl}=1.0$, $\lambda_{cons}=1.0$, $\lambda_{bd}=0.5$
\item Data augmentation: weak augmentation (teacher) + strong augmentation (student)
\end{itemize}

\subsection{Main Results}

\begin{table}[htbp]
\caption{Comparison with state-of-the-art methods on EndoVis 2017 (20\% labeled data)}
\begin{center}
...
Figure~\ref{fig:risk_map} visualizes the risk maps generated by our method. The high-risk regions (red) correspond to instrument boundaries and ambiguous areas where depth discontinuities coincide with prediction uncertainty. The low-risk regions (green) are primarily in the interior of instruments and background areas where pseudo-labels are reliable.

\subsubsection{Effect of Labeled Data Ratio}
Table~\ref{tab:baseline_comparison} shows that GeoRisk-SPC consistently outperforms baselines across different labeled data ratios. The improvement is more significant with less labeled data: at 10\% labeled ratio, GeoRisk-SPC achieves 88.59\% Dice compared to MT's 85.17\% (+3.42\%), while at 40\% labeled ratio, the improvement is 91.15\% vs 86.46\% (+4.69\%). This demonstrates that our method is particularly effective in low-label regimes where pseudo-label quality is more critical.
...
Experiments on EndoVis 2017 demonstrate that GeoRisk-SPC achieves 92.15\% Dice with 20\% labeled data, 91.15\% Dice with 40\% labeled data, and 88.59\% Dice with only 10\% labeled data, consistently outperforming existing methods including MT, UAMT, and URPC across all settings. The improvement is particularly significant in low-label regimes, demonstrating the effectiveness of our approach when annotation resources are limited.
