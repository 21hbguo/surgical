...
\begin{abstract}
Semi-supervised surgical instrument segmentation reduces annotation costs but suffers from noisy pseudo-labels in ambiguous regions. We propose GeoRisk-SPC, a geometry-aware risk localization method that uses depth discontinuity as structural prior to identify high-risk regions in pseudo-labels. Combined with teacher uncertainty and geometry-semantic conflict detection, our method partitions the unlabeled data into low-risk and high-risk regions. Low-risk regions receive hard pseudo-label supervision, while high-risk regions undergo structural perturbation consistency learning. Experiments on EndoVis 2017 (4-fold cross-validation) demonstrate that GeoRisk-SPC with UNet achieves 91.15\% Dice with 40\% labeled data and 88.59\% Dice with 10\% labeled data, outperforming MT, UAMT, and URPC across all settings. With the DepthGuiderV4 variant, GeoRisk-SPC+DGv4 further achieves 91.41\% Dice with 40\% labeled data and 88.26\% Dice with 5\% labeled data. The improvement is particularly significant in low-label regimes: at 5\% labeled ratio, GeoRisk-SPC achieves 86.68\% Dice compared to MT's 84.27\% (+2.41\%).
\end{abstract}
...
\item \textbf{Comprehensive evaluation}: Experiments on EndoVis 2017 demonstrate consistent improvements across multiple labeled data ratios (5\%, 10\%, 20\%, 40\%), with GeoRisk-SPC achieving 88.72\% Dice with 20\% labeled data, outperforming existing methods by significant margins.
\end{itemize}
...
\begin{equation}
B_p = \|S * \hat{y}_{argmax}\|
\end{equation}
where $\hat{y}_{argmax}$ is the argmax of teacher predictions.
...
High-risk consistency loss (teacher prediction is detached to prevent collapse):
\begin{equation}
\mathcal{L}_{cons} = \frac{1}{|M_r|} \sum_{(i,j) \in M_r} \text{KL}(\text{softmax}(z_{pert}^{(i,j)}) \| \text{sg}[\text{softmax}(z_{clean}^{(i,j)})])
\end{equation}
where $z$ denotes logits, $\text{sg}[\cdot]$ denotes stop-gradient.
...
Table~\ref{tab:main_results} compares GeoRisk-SPC with baselines across different labeled data ratios on EndoVis 2017 (4-fold cross-validation). Methods are sorted by publication year. GeoRisk-SPC+DGv4 achieves competitive results, with 91.41\% Dice at 40\% and 90.90\% at 20\%. Compared to SegMatch (2025), GeoRisk-SPC+DGv4 achieves comparable or better results at higher label ratios (40\%), while SegMatch performs well at lower ratios. Note that GeoRisk-SPC uses depth information as geometric prior, while all baselines use RGB-only input.
...
CPS \cite{chen2021cps} & 2021 & -- & -- & 89.88{\tiny$\pm$9.82} & -- \\
...
\midrule
Fully Supervised & -- & -- & -- & -- & 86.67{\tiny$\pm$13.97} \\
\midrule
...
{\footnotesize $^{\dagger}$Simplified reproduction using EMA teacher with noise/adversarial augmentation. Official implementations require dual strong branches (UniMatch) or spatial weak augmentation with inverse transform (SegMatch).}
\end{table}
...
The standard deviations in Table~\ref{tab:main_results} show that GeoRisk-SPC achieves more stable performance across folds compared to baselines. For example, at 40\% labeled ratio, GeoRisk-SPC has std of 8.91\% while MT has 15.11\%; at 5\% labeled ratio, GeoRisk-SPC+DGv4 has std of 12.75\% while MT has 15.51\%. This suggests that our risk localization mechanism provides more consistent supervision across different data splits.
