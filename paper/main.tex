...
Table~\ref{tab:main_results} compares GeoRisk-SPC with baselines across different labeled data ratios on EndoVis 2017 (4-fold cross-validation). Methods are sorted by publication year. GeoRisk-SPC-DG achieves the best mean Dice among all methods at 10\%, 20\%, and 40\% labels, and matches the strongest baseline at 5\%. Compared to UniMatch, GeoRisk-SPC-DG improves by +0.90\% at 5\%, +2.24\% at 10\%, +1.04\% at 20\%, and +2.19\% at 40\%. Compared to SegMatch, the improvements are +0\% (match), +1.33\%, +0.73\%, and +1.42\% respectively. Notably, GeoRisk-SPC-DG with only 10\% labels (90.42\%) surpasses SegMatch with 40\% labels (89.99\%), demonstrating superior label efficiency through geometry-aware risk supervision.
...
{\footnotesize Reproduced under the same data splits, training budget, and evaluation protocol. Sup-only uses only labeled data without unlabeled supervision.}
\end{table}
