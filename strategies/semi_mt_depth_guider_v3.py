"""方案简介（MT-DepthGuider-v3）：
work！
使用全局+局部 depth guider 在编码器各层对 RGB 特征做门控残差增强。
...
class MTDepthGuiderV3Strategy(BaseTrainingStrategy):
    def __init__(self, args, model, optimizer, device):
        ...
        if int(getattr(args, "use_depth", 0) or 0) not in (1, 13):
            raise ValueError("mt_depth_guider_v3 requires --use_depth 1 or 13 (depth1c guider input).")
