...
from strategies.semi_mt_depth_guider_proto_v1 import MTDepthGuiderProtoV1Strategy
...
class MTDepthGuiderProtoV1StrategyTest(unittest.TestCase):
    def setUp(self):
        self.args = Namespace(
            consistency=0.1,
            grad_clip=0.0,
        ...
        self.strategy = MTDepthGuiderProtoV1Strategy(self.args, self.model, self.optimizer, self.device)

    def test_strategy_registered(self):
        self.assertIn("mt_depth_guider_proto_v1", STRATEGY_REGISTRY)

    def test_compute_loss_returns_proto_terms_with_teacher_pred(self):
        batch = {
        ...
        self.assertEqual(
            set(loss_dict.keys()),
            {"total", "ce", "dice", "proto", "proto_weight", "consistency", "consistency_weight"},
        )
    ...
    def test_requires_depth1c(self):
        ...
        with self.assertRaises(ValueError):
            MTDepthGuiderProtoV1Strategy(args, self.model, self.optimizer, self.device)
